...
**2026-08-27 — Security Wave 1 applied (DEFINER RPCs).** Shared production app DB locked: photo/cron/Firebase-dump RPCs are service_role-only; DM/unread IDOR bound to `auth.uid()`; trigger helpers reject direct RPC. Branch `fix/security-definer-rpcs`, migration `20260827134743_lock_dangerous_definer_rpcs`. SQL-only — no OTA. Next wave: storage policies.
