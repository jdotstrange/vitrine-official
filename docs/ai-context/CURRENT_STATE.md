...
## Current Priority
**2026-05-26 — Drag-Reorder V2 Migration on branch `feature/drag-reorder-v2` (pending dev-client validation + PR merge).** Migrated the upload Scan-step photo grid from `react-native-draggable-flatlist` to `react-native-reanimated-dnd@^2.0.0` AND extracted the implementation into a canonical vault primitive (`PhotoReorderGrid`) so every future multi-photo reorder surface consumes the same component. Closes the Layer-2 drag-reorder polish thread that's been open since the May 24 crash chain. The new primitive owns the lift visual (scale 1.12, inner glow + brandVolt border, no shadow), live COVER badge re-anchoring, remove-X disable during drag, haptic wiring, and the 220ms long-press threshold. Cross-platform-consistency-first principle codified in DECISION_LOG and the design-system playbook (the "no shadows in lift visuals" rule). **Migration required bumping `react-native-reanimated` 4.1.7 → 4.3.1 and `react-native-worklets` 0.5 → 0.8** (both native modules) — this is a binary rebuild, NOT OTA-eligible from the current preview binary. Rollout path: founder dev-client validation on iOS + Android → PR merge → `eas build --profile preview` → 24h soak → future JS iterations OTA on top of that new binary. DFL was NOT removed from `package.json` because the legacy V1 memorabilia upload flow (`upload/memorabilia-core-form.tsx` → `upload/photo-grid.tsx` at `/upload/memorabilia/[type]/[category]`) still depends on it; that V1 → `PhotoReorderGrid` migration is a separate follow-up thread.
