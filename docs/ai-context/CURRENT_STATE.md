...
3. **97% hang on single-lane uploads** — intermittent (most uploads complete fine; signed items with no context provided seem most affected). Plan: Sentry-instrument `upload-entry.tsx` to capture timing + extraction status at hang points, then diagnose. Not started.
