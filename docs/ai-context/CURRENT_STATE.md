...
## Current Build Phase
Active V3 redesign and feature build-out, hosted in a pnpm + Turborepo monorepo with two apps (`@vitrine/native`, `@vitrine/web`) and four shared workspace packages (`@vitrine/design-tokens`, `@vitrine/constants`, `@vitrine/types`, `@vitrine/api`). All primary native surfaces are V3. The web app is now also fully V3-aligned: a single-page marketing site rendering the canonical V3 dark / frost-on-void aesthetic, plus matching share-resolver pages. Approaching deployment readiness with a five-lens profile hub, four-lens Tracking Hub, Instagram-style Market Surface, dedicated Messages tab, V3 Settings, Light/Dark/Auto theme system, and extensive backend infrastructure (Edge Functions, cron jobs, RPC functions).

## Current Priority
Day 2 of the monorepo migration is complete (shared packages live). The web marketing site V3 rebuild is also complete (single-page, brand-correct, mobile-responsive, with re-skinned share resolvers). Next focus splits into three tracks — pick whichever matters most for product:
1. **Marketing copy + narrative iteration** on the now-shipped V3 site (the structure is right; the copy was ported verbatim from the mockup and explicitly deferred for later iteration).
2. **EAS / TestFlight pipeline** (`docs/EAS_MIGRATION_PLAN.md`, `docs/TESTFLIGHT_CHECKLIST.md`) — gating any real device distribution.
3. **Product features** — Crown Jewel detail-screen assignment UI; AI upload flow QA pass.
...
### Web Marketing Site (V3 — Live)
Single-page V3-aligned marketing site (`apps/web/app/page.tsx`) rendering 20 ported sections from the `vitrine-2026` HTML mockup, all brand-correct, mobile-responsive, with re-skinned share resolvers. Down from the previous 12-route multi-page build (about/pricing/identity/features/contact/explore + ~40 home/about/pricing/features components, all deleted).
- **Routes**: `/`, `/_not-found`, dynamic `/icon`, `/apple-icon`, `/opengraph-image`, plus the three share resolvers `/s/c/[id]`, `/s/p/[id]`, `/s/s/[id]`. Six 301 redirects in `next.config.mjs` map legacy URLs to `#anchor` sections of `/`.
- **Design system**: `apps/web/app/globals.css` `:root` consumes `@vitrine/design-tokens` `DARK_COLORS` directly. Marketing-specific vars (`--fg1/2/3`, `--frost-divider/border(-strong)`, `--brand-volt(-fill/-border)`, semantic + trait color sets, `--sheet-bg`, `--press-overlay`, `--scrim`) live alongside re-pointed shadcn vars so future authenticated UI keeps working. `apps/web/lib/marketing/tokens.ts` exports a `T` object resolving these as `T.void` / `T.volt` etc. — the inline-style bridge that lets section JSX read 1:1 with the mockup while every reference flows through tokens.
- **Fonts**: Electrolize (display, `T.fontDisplay`), Space Grotesk (`T.fontGrotesk` for kickers/labels), Inter (`T.fontInter` body), Libre Caslon Text + italic (`T.fontCaslon` for accent italics in headlines/subtitles), JetBrains Mono (`T.fontMono` for stats/values). All loaded via `next/font/google` in `apps/web/app/layout.tsx`.
- **Brand assets**: Canonical wordmark at `apps/web/public/logo.svg` (sourced from `apps/native/assets/logo.svg` with `fill="currentColor"`). React wrappers `apps/web/components/marketing/{VitrineLogo,VitrineMark}.tsx`. Standalone crown mark mirrors `apps/native/components/vault/icons/vitrine-mark-icon.tsx`. Both consume shared SVG path data from `apps/web/lib/marketing/brand-paths.ts` (`VITRINE_MARK_VIEWBOX` + `VITRINE_MARK_PATHS`) so the dynamic `/icon`, `/apple-icon`, `/opengraph-image` endpoints render the same mark — single source of truth.
- **Section architecture**: `apps/web/components/marketing/MarketingSite.tsx` composes 20 section files in `apps/web/components/marketing/sections/`: SiteNav, Hero, Pulse, Problem, Thesis, Intelligence (4.8s loop), Cataloging (CARD/WATCH/COMIC/SNEAKER/COIN tabs), Showcases (parallax), Tracking (SVG chart), Comps (3-tier), Community, Categories (6×6 grid), HowItWorks, LiveComing, Explore (4×2 grid), Pro (VAR/AAR/MarketPulse/AutoShowcase), Press, FAQ, FinalCTA, Footer. Plus shared SectionHeader. Inline styles preserved per-section (the mockup encodes hundreds of micro-decisions like `letterSpacing: -1.4`); all colors/fonts/spacing flow through `T` so token consistency holds without rewriting to Tailwind.
- **Primitives** (`apps/web/components/marketing/primitives/`): `FrostCard`, `Pill` (status variants: `for_sale`, `for_trade`, `sell_trade`, `nfst`, `rookie`, `signed`, `game_used`, `graded`, `volt`), `Kicker`, `MIcon` (lucide-react wrapper with kebab→Pascal helper), `AppStoreBadge`, `PulseRow`, `Button`, `CompRow`, `HolographicFrame`, `GradientVeil`. Most are `"use client"`.
- **Motion + utility hooks** (`apps/web/lib/marketing/`): `hooks.ts` (`useTicker`, `useDriftingPrice`, `usePulseFeed`, `usePrefersReducedMotion`, `useInView`, `useScrollProgress`, `useScrollY`, `useActiveSection`, `timeAgo`), `Reveal.tsx`, `Stagger.tsx`, `Parallax.tsx`. Plus static modules `photos.ts` (8 Unsplash placeholder URLs), `constants.ts` (`SCHEMAS`, `CATS`, `INTEL_STAGES`, `KICKER_CYCLE`, `PULSE_TEMPLATES`).
- **Mobile responsive**: Two-tier breakpoint layer in `globals.css`. ≤1024px (tablet): hero stacks, h1 92→64, 4-col grids → 2-col, 6-col → 3-col, all two-pane splits → stacked, SectionHeader collapses. ≤768px (phone): all multi-col grids → 1-col, hero phone hidden (type-led), h1 → 44, hamburger nav (`MobileNav.tsx` slide-down panel), Categories 3→2-col, Explore filters horizontal scroll. Driven entirely by `data-marketing-*` attributes on section components — keeps breakpoint logic in one file. All overrides use `!important` (inline-style specificity).
- **Share-resolver design**: `apps/web/components/share/share-landing.tsx` mirrors the marketing site aesthetic — pure void background with grid + warm halo, wordmark header bar, Kicker → image card → Electrolize title → italic Caslon subtitle → JetBrains Mono stats → ivory CTA pill (`cta-glow`) → fine print. 404 state has oversized volt numeral. Direct Supabase queries in `/s/c/[id]`, `/s/p/[id]`, `/s/s/[id]` unchanged — only the UI layer was rewritten. Mobile responsive via `data-share-*` attributes.
- **Dynamic icons + OG**: `apps/web/app/{icon,apple-icon,opengraph-image}.tsx` use `next/og` `ImageResponse` on the Edge runtime. Icon 64×64 ivory crown on void; apple-icon 180×180 same; OG 1200×630 with mark + ELECTROLIZE wordmark + headline + tagline + warm halo. All three import from `brand-paths.ts` so brand changes propagate in lockstep.
- **Key files**: `apps/web/app/{page,layout,not-found,globals.css,next.config.mjs,icon,apple-icon,opengraph-image}.tsx`, `apps/web/components/marketing/**`, `apps/web/lib/marketing/**`, `apps/web/components/share/share-landing.tsx`.
