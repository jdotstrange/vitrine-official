...
- **Mobile responsive**: Two-tier breakpoint layer in `globals.css`. ≤1024px (tablet) and ≤768px (phone). Driven entirely by `data-marketing-*` attributes on section components — keeps breakpoint logic in one file. All overrides use `!important` (inline-style specificity).
