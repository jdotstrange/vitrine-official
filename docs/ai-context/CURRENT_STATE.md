...
**2026-05-30 — Preview runtime `2` committed; binary cut pending.** `apps/native/app.json` `runtimeVersion: "2"` on `main` (`33ec04f`, pushed). Isolates preview channel from runtime-`1` installs that cannot run `PhotoReorderGrid`. **Founder action:** `eas build --profile preview --platform ios` from `apps/native/` → distribute IPA → team reinstall. No Android preview builds exist in EAS history. **2026-05-26 — PhotoReorderGrid on `main`.** Upload Scan uses `<PhotoReorderGrid />`; requires Reanimated 4.3.1 + Worklets 0.8 in binary. DFL remains for V1 memorabilia. Next after binary ships: Batch Lane Review tab as next `PhotoReorderGrid` consumer.
