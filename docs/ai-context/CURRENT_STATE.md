...
## Current Priority
**2026-05-27 — Upload + detail polish on `main` (preview OTAs shipped).** Three JS-only waves landed on `preview` (runtime `1`), all founder-validated on dev client:
1. **Assembly + deferred variants** (`62d8222`) — Scan → Theater → Review → Finalize → **Assembly** (dossier seals Direction B) → Success; variants via `assemblyVariants()` after commit, not at Identify.
2. **LensPager page-0 edge-back** (`5d32845`, OTA `a3610490-8612-4e9f-858f-ece6e2ca932b`) — asymmetric `activeOffsetX` on first lens so stack swipe-back works in collectible detail middle content without a back chevron in the display `LensSelector`.
3. **Theater 25s linear / 85% cap** (`f09e891`, OTA `7356da1c-9b2c-4a34-b3d3-486c07796c54`) — cosmetic ring starts after `extractionJobId`; linear crawl; percent capped until extraction completes; poll success still sprints to 100%.

**2026-05-30 — `runtimeVersion` `2` for new preview binary.** Bumped in `apps/native/app.json` so preview OTAs no longer target runtime-`1` installs (Reanimated 4.1 / no `reanimated-dnd` — upload crash risk). Founder cutting `eas build --profile preview` for team distribution. **2026-05-26 — PhotoReorderGrid on `main`.** Upload Scan uses `<PhotoReorderGrid />`; requires Reanimated 4.3.1 + Worklets 0.8 in binary. DFL remains for V1 memorabilia. Next after binary: Batch Lane Review tab as next `PhotoReorderGrid` consumer.
...
Parallel tracks:
1. **Drag-reorder Layer 2** — ~~migrate from `react-native-draggable-flatlist@4.0.3` to `react-native-reanimated-drag-list`~~ **DONE 2026-05-26** as the bigger "PhotoReorderGrid extraction" effort on branch `feature/drag-reorder-v2`. Pending founder dev-client validation + PR merge + EAS preview rebuild. Successor track: migrate V1 memorabilia photo grid to `PhotoReorderGrid` to fully retire DFL (see OPEN_THREADS).
...
3. **97% hang on single-lane uploads (extraction-reliability slice)** — intermittent Theater stall at cosmetic 97% (real exit is extraction poll). **Variant-contention slice addressed 2026-05-27** via Assembly + deferred variants. If rapid-fire repro persists, next plan is Theater timeouts / Realtime / webhook reconciliation — not variant work at Identify.
...
- **Pipeline**: Scan → Theater (Looking Glass HUD) → Review → Finalize → **Assembly** (deferred image variants) → Success. Tapping "Identify" uploads originals only (no variant generation); transitions to Theater immediately; real async extraction via `enqueue-extraction` Edge Function + Looking Glass engine; status tracked via 2s polling on the collectibles row; `complete` (or legacy `extracted`) triggers cascade-complete + 1s pause + transition to Review. Scan, Review, and Finalize use the `ActionDock` commit-action pattern. **Finalize still calls `commitDraftCollectible`** (showcase/tags/status/value); Assembly gates Success until variants settle or 45s timeout. **Server-side auto-publish**: the `complete_and_publish` trigger sets `published_at` when extraction succeeds (unless `batch_uploads.auto_publish = false`) — separate from the client Finalize commit that sets listing metadata.
- **Capture screen (Scan)**: Dynamic 3-column photo grid via the **canonical `<PhotoReorderGrid />` primitive** (`components/vault/photo-reorder-grid.tsx`, built on `react-native-reanimated-dnd@^2.0.0`). Grid starts with a single `+` tile (rendered OUTSIDE the SortableGrid as an absolutely positioned sibling, not draggable, not a drop target), grows as photos are added, caps at 6. Per-tile long-press 220ms lifts that specific tile (Pattern A — matches iOS Photos). Lift visual: scale 1.12 spring (`damping:18, stiffness:220`), inner glow `rgba(255,255,255,0.06)` + brandVolt border 1→2px (NO shadows / NO elevation — cross-platform consistency rule). Other tiles shuffle aside via the library's Insert strategy (the void IS the drop indicator). COVER badge re-anchors live to whichever tile is at grid index 0 during drag (preview-the-decision affordance). Remove-X is disabled (opacity 0.4, pointerEvents none) while any tile is in drag state. Haptics: selection on drag start, light impact on each placeholder move, medium impact on drop. Empty-tile tap opens `ActionSheet` with Take Photo (single shot) / Choose from Library (batch via native `PHPickerViewController` through `ImagePicker.launchImageLibraryAsync` with `allowsMultipleSelection`, `selectionLimit`, `orderedSelection: true`). Camera permission requested with deep-link to Settings on denial. Context input (90 char cap) wraps in `KeyboardSafeScroll` so the keyboard doesn't cover it. ActionDock with "Identify" label pinned at bottom.
- **State reset on focus**: `resetFlow` clears photos, selected showcases (`selectedShowcaseIds` + `localShowcases`), tags, status, visibility, and `estimatedValue` so showcase state doesn't leak across uploads. Showcase list is refetched via `useFocusEffect` on tab focus so the picker always reflects the current showcase list. Finalize screen renders selected showcases as removable chips (mirroring the tag UX).
- **Theater (Looking Glass HUD)**: Holographic-framed hero, gradient progress ring, 5-item checklist (4×4s + 9s = 25s) with monochrome brandVolt completion color. Cosmetic ring/reveal run **25s linear** to **85% cap** (`THEATER_PROGRESS_CAP`); animation starts once `extractionJobId` exists. Percent label floors and caps at 84 until `extracted`/`complete`, then poll success sprints to 100% (250ms). Image fades 0 → 0.5 over the same 25s window. Real exit remains poll-driven — early extraction completion still accelerates to Review.
