...
## Current Priority
**2026-08-27 — Security Waves 4+5 applied.** Dictionaries + `view_counters`: RLS on, SELECT for anon/authenticated. Service-role-only tables RLS on with no client policies. 17 functions `search_path` pinned (including `complete_and_publish` body untouched). `collectibles_unified` `security_invoker`. Migrations `20260827174927` + `20260827174937`. SQL-only — no OTA. **Wave 6** still the leftover `collectible_field_values` drop. HIBP + policy initplan/dedupe deferred (Auth dashboard / 40+ policies).

**2026-08-27 — Security Wave 3 applied (`collectible_field_values` RLS).** Writes owner-only via `owns_collectible()` (profile id, not `auth.uid()`). SELECT mirrors parent `collectibles` visibility — not owner-only (that would blank visitor Specs / comps v2). Specs UI reads `ai_metadata` JSONB. Migration `20260827171954`. SQL-only — no OTA. **Wave 6 (after 4–5):** drop this leftover table — see `CODE_HYGIENE.md`.
