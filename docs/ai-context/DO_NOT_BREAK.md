...
## Critical User Flows
- Auth via Supabase email OTP — **unified `components/auth-screen.tsx`** (one email→6-digit-OTP screen for login AND signup, `shouldCreateUser: true`). `app/login` renders it; `app/signup` redirects to `/login`. `login-page.tsx`/`signup-page.tsx` are deleted — do not resurrect a password fork.
- Boot screen launch handoff — `components/vitrine-boot-screen.tsx` shows while `auth.isLoading`, reusing native splash art for a flash-free transition.
...
- **Keyboard handling uses the three vault wrappers, NOT raw `KeyboardAvoidingView`.** All new input surfaces MUST use `KeyboardSafeScroll` (forms/scrollable lists), `KeyboardSafeSheet` (modals/sheets), or `KeyboardSafeComposer` (sticky composer bars) from `@/components/vault`. The wrappers own offset / behavior / keyboard-toolbar contracts; consumers don't reinvent. The non-existent `automaticOffset` prop has been removed from 23 surfaces — don't re-introduce it. **The global `<KeyboardToolbar />` (Prev/Next/Done bar) was removed from `app/_layout.tsx` on 2026-06-22** — `upload-entry.tsx` keeps its own scoped `InputAccessoryView`. Don't re-mount a global toolbar without product approval.
- **`PhotoReorderGrid`'s `gridKey` remount is load-bearing.** `SortableGrid` is keyed on the sorted photo id set because `react-native-reanimated-dnd`'s `useGridSortable` snapshots each item's slot on first render and its correction reaction skips the `undefined → value` transition. Without the key, a photo appended to an already-mounted grid renders stacked on the cover tile and looks like it was never added (founder-reported 2026-07-29). Do NOT key on `photos.length` (misses same-length swaps) and do NOT key on the ordered ids (a drag-reorder would remount mid-spring). If `react-native-reanimated-dnd` ever fixes the guard upstream, this can go — verify by adding photos one at a time to a non-empty grid.
...
- **Never dispatch state from inside a functional setState updater.** React 19 (RN 0.81) eagerly evaluates updaters inside the dispatch when the fiber is idle; a nested dispatch enqueued during that evaluation gets overwritten when the outer updater returns unchanged state. This silently dropped every photo add/remove after the first in the upload flow (2026-07-29). `appendPhotos` / `removePhoto` in `upload-entry.tsx` now read the `photos` closure and dispatch once via `requestPhotoUpdate` — do not "simplify" them back into `setPhotos((current) => { … dispatch … })`. Known remaining instance: `tracking-hub.tsx:170` (see OPEN_THREADS).
- **Only `file://` URIs may reach `uploadImage` / `readUriAsArrayBuffer`.** `expo-file-system`'s `File` constructor throws (`validatePath`, `FileSystemFile.swift:48`) for anything that isn't a plain on-disk file — remote URLs included. Pickers must run `materializeLocalImageUri` first (`content://` / other non-file URIs get copied through ImageManipulator into cache; already-`file://` URIs return as-is — iOS path is unchanged). Two additional guards keep the storage path true and both are load-bearing: the speculative-upload effect in `upload-entry.tsx` skips `isRemotePhotoUri(photo.uri)` (edit mode seeds `photos` with Supabase `https://` storage URLs), and it records each photo id in `speculativeAttemptedRef` so a failed upload can't be re-fired every render. Removing either resurrects REACT-NATIVE-12 as a permanent retry loop. `compressImage` may fall back to its input URI only when that input is already a local file.
- **Do NOT add `livePhotos` to `mediaTypes` in the upload picker.** Requesting only `['images']` is what makes iOS flatten a Live Photo to a still JPEG in cache for us. Opting in returns the original uncompressed frame (the `quality` option is ignored by design so the frame can re-pair with its video) plus a paired video file, which moves full-resolution downsizing onto the device at exactly the moment memory is tightest. Rationale is commented at the call site.
...
- **OTP autofill depends on a SINGLE `TextInput` with `textContentType="oneTimeCode"` + `autoComplete` in `auth-screen.tsx`.** Do NOT split the code field into six separate boxes — that breaks iOS autofill (the reason the original boxes were removed). Auto-submit fires on 6 digits.
- **Boot splash continuity is load-bearing.** `lib/splash-contain-layout.ts` (`SPLASH_BG`, `SPLASH_SOURCE`, `getContainRect`) must stay matched to `app.json` `splash` (`backgroundColor: "#020202"`, `image: splash-icon.png`, `resizeMode: contain`). The native splash is hidden **inside `components/vitrine-boot-screen.tsx`** on mount — do NOT re-add `SplashScreen.hideAsync()` to `app/_layout.tsx` (it flashes the stack). Change `app.json` splash and the layout file together.
- **Skeleton imports go through the `components/skeleton/` barrel.** Legacy `components/{skeleton,skeleton-community,skeleton-messaging}.tsx` and dead `components/skeletons/{connections,detail,edit-profile,group-info,inbox,notifications,profile,showcase,thread,tracking,upload}.tsx` are DELETED. New skeletons import from `@/components/skeleton`; composed screen skeletons live in `components/skeletons/{collectible-detail,profile-hub,showcase-detail,tracking-overview}.tsx`. Use the shared `SkeletonPulseProvider` (`components/vault/skeleton.tsx`) — don't add per-component animation loops.
- **Supabase auth email templates are NOT shipped by OTA.** `supabase/templates/auth/email-otp.html` lives in the repo for version control only — it must be pasted manually into Supabase Dashboard → Authentication → Email Templates (Magic Link / OTP), and the email logo must be uploaded to Storage `brand-assets/logos/icon.png` (public). Editing the repo file alone changes nothing in production.
- **`runtimeVersion` in `apps/native/app.json` is `"3"` (Android-first + `expo-clipboard` as of 2026-08-14).** After any native dep / plugin / permission change: bump runtime again and rebuild. Preview OTAs apply **only** to installs whose embedded runtime matches. Do NOT OTA runtime-`3` JS (imports `expo-clipboard`) onto runtime-`2` iOS preview IPAs — those need a fresh IPA.
