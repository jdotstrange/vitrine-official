...
**Status: Waves 1–3 applied (2026-08-27).** Wave 1 DEFINER RPCs (`20260827134743`, PR #4). Wave 2 storage (`20260827140913`, PR #5). `user_category_interests` dropped (`20260827150214`, PR #6). Wave 3 `collectible_field_values` RLS (`20260827171954`, PR #7) — owner writes via `owns_collectible()`; SELECT follows parent collectibles RLS. **Next: Wave 4** dictionaries + service-role-only tables. **Wave 6 (after 4–5):** drop `collectible_field_values` — leftover filing cabinet, no new writes since 2026-03-10; see `CODE_HYGIENE.md`. Canvas: `canvases/supabase-security-audit.canvas.tsx`.
