...
### Theater 1 extraction reliability (97% cosmetic stall — extraction axis)
Separate from the 2026-05-27 variant-deferral fix. When polling never sees `extracted`/`complete`, the Theater ring sits at 97% (cosmetic `withTiming` cap). Remaining levers: Theater hard timeout + failed UX, Supabase Realtime on `collectibles.extraction_status`, webhook retry/reconciliation in `looking-glass-webhook`, wiring `raceForCompletion` into Theater. Track if founder rapid-fire repro persists after Assembly ships.
...
- ~~Upload photo grid drag-reorder polish (Layer 2)~~ → Resolved 2026-05-26 on branch `feature/drag-reorder-v2`. Migrated from `react-native-draggable-flatlist@4.0.3` to `react-native-reanimated-dnd@^2.0.0` and **extracted the implementation into `PhotoReorderGrid`** at `apps/native/components/vault/photo-reorder-grid.tsx`. Net upload-entry.tsx delta: ~140-line deletion, ~10-line addition. New aesthetic spec (lift scale 1.12, inner glow + brandVolt border, no shadow, COVER live-anchors, remove-X disables during drag, per-tile long-press 220ms) lives inside the primitive. **The migration required bumping `react-native-reanimated` (4.1.7 → 4.3.1) and `react-native-worklets` (0.5 → 0.8)** — both native modules — so it's a binary rebuild, not an OTA. DFL retained in `package.json` because legacy V1 memorabilia flow still uses it (see new follow-up thread above). See DECISION_LOG ("`PhotoReorderGrid` is the canonical multi-photo reorder primitive" + "Cross-platform consistency first") and IMPLEMENTATION_LOG ("Drag-Reorder V2 Migration") for full detail. Pending: founder dev-client validation on iOS + Android, PR merge, `eas build --profile preview --platform ios|android`, then 24h soak before any further OTA iterations.
