...
**Status: Waves 1–2 applied on prod (2026-08-27).** Wave 1 DEFINER RPCs (`20260827134743`, PR #4). Wave 2 storage (`20260827140913`): INSERT/UPDATE/DELETE on `collectible-images` and `user-avatars` bound to `current_profile_id()` (`public.users.id`, not `auth.uid()`); migrated/`{collectible_id}` DELETE via `owns_collectible`; message-attachments client writes path-bound (edge `media-upload` still service_role). **No OTA.** Smoke: upload a collectible photo, change avatar, delete an item (storage cleanup). **Next: Wave 3** RLS on `collectible_field_values` + `user_category_interests`. Canvas: `canvases/supabase-security-audit.canvas.tsx`.
