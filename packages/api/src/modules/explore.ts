/**
 * Explore API — Discover lens RPCs (hot items, new listings, for-sale,
 * featured collectors) plus the generic browseCollectibles RPC.
 *
 * Self-contained: includes a local `deriveStatus` helper rather than
 * depending on the (native-only) `lib/design/status-config` module.
 */
...
function deriveStatus(forSale: boolean | null | undefined, forTrade: boolean | null | undefined): ListingStatus {
...
export interface ExploreApi {
  getHotItems(limit?: number, excludeUserId?: string): Promise<HotItem[]>;
  getExploreCategories(): Promise<ExploreCategory[]>;
  getNewListings(limit?: number, excludeUserId?: string): Promise<NewListing[]>;
  getForSaleNow(limit?: number, excludeUserId?: string): Promise<ListedItem[]>;
  getCollectorsToFollow(excludeUserId?: string, limit?: number): Promise<FeaturedCollector[]>;
  browseCollectibles(filters?: BrowseFilters): Promise<BrowseResult[]>;
}

export function createExploreApi(supabase: SupabaseClient, logger: Logger): ExploreApi {
  ...
  async function getHotItems(limit = 8, excludeUserId?: string): Promise<HotItem[]> {
  ...
  async function getExploreCategories(): Promise<ExploreCategory[]> {
    ...
    if (countsRes.error) {
      log.warn(
        'Category counts unavailable; returning categories without counts:',
  ...
  async function getNewListings(limit = 8, excludeUserId?: string): Promise<NewListing[]> {
    let query = supabase
      .from('collectibles')
      .select(
  ...
  async function getForSaleNow(limit = 8, excludeUserId?: string): Promise<ListedItem[]> {
    let query = supabase
      .from('collectibles')
      .select(
  ...
  async function getCollectorsToFollow(
    excludeUserId?: string,
    limit = 10,
    ...
    if (previewError) log.warn('Error fetching preview images:', previewError);
  ...
  async function browseCollectibles(filters: BrowseFilters = {}): Promise<BrowseResult[]> {
