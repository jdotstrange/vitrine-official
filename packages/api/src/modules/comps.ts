...
/** Pure helper — exported so call sites can use it without instantiating the API. */
...
export function createCompsApi(supabase: SupabaseClient, logger: Logger) {
  const log = logger.create('API:Comps');

  async function getCollectibleComps(sourceId: string, limit = 30): Promise<CompItem[]> {
  ...
  async function getTrackedComps(userId: string, limit = 30): Promise<TrackedCompItem[]> {
...
export type CompsApi = ReturnType<typeof createCompsApi>;
