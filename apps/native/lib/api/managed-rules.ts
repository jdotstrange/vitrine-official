 ...
 * intentionally pure: no React, no Supabase, no `@/lib/...` aliases. That
 * shape lets it be imported by:
 *   - The client rule builder (`components/managed-rule-builder.tsx`) for the
 *     live preview.
 *   - The `managed-evaluate` Edge function for immediate evaluation when the
 *     user saves a new/updated rule.
 *   - The `managed-sweep-worker` Edge function for the cron-driven incremental
 *     and nightly sweeps.
 *
...
// ---------------------------------------------------------------------------
//
// The rule grammar lives in @vitrine/types so the client builder, web RSC,
// and Edge function evaluators all reason about the same shape. Re-exported
// here so existing `import { type ManagedRules } from '@/lib/api/managed-rules'`
// stays byte-identical. The EVALUATOR (itemMatchesManagedRules etc.) lives
// below in this file until Day 2.5 moves it into @vitrine/api.
...
/**
 * Evaluate a single condition against a normalized collectible row.
 * Returns false (not match) for any null-side comparison except where the
 * semantics explicitly allow it (e.g. an empty `traits` array is matchable
 * by `is_none_of`).
 */
 ...
 * Empty `conditions` is treated as "matches nothing" (consistent with the
 * "≥1 condition before save" guardrail) — calling code that wants the
 * preview-friendly "match everything" shape should branch on
...
export function itemMatchesManagedRules(item: EvalCollectible, rules: ManagedRules): boolean {
  ...
  // Default to 'all' for any other value (validator will catch invalid modes elsewhere).
...
export function evalRowFromCollectionItem(item: {
  ...
  status: string;
  traits: string[];
  /** `CollectionItem` doesn't currently expose tags; pass through if the caller has them. */
  tags?: string[];
